...
f2b <- ggplot(data = A7_catchN) +
  ...
  theme_bw(18) +
  coord_cartesian(ylim = c(0, 2.5)) +
  xlab("") + ylab("Relative catch in number - Captura relativa en número")
...
f3 <- ggplot(data = A7) +
  ...
  theme_bw(18) +
  coord_cartesian(ylim = c(0, 2.5)) +
  xlab("") + ylab("Relative catch in weight per set - Captura por lance relativa en peso")
...
```{r Figure 5a}
Figure5a <- "D:/OneDrive - IATTC/IATTC/2026/SAC17/DEL SSIs/DEL_YFT_EPO_sdmTMB_Poisson/"
...
```{r Figure 5b}
Figure5b <- "D:/OneDrive - IATTC/IATTC/2026/SAC17/DEL SSIs/"
...
```{r Figure 7top}
BET_Index <- "D:/OneDrive - IATTC/IATTC/2026/SAC17/LL SSIs/Joint index/BET/JPN_KOR_BET/"
BET_Index_JPN <- "D:/OneDrive - IATTC/IATTC/2026/SAC17/LL SSIs/Joint index/BET/JPN_BET/"
BET_Index_KOR <- "D:/OneDrive - IATTC/IATTC/2026/SAC17/LL SSIs/Joint index/BET/KOR_BET/"
YFT_Index <- "D:/OneDrive - IATTC/IATTC/2026/SAC17/LL SSIs/Joint index/YFT/JPN_KOR_YFT/"
YFT_Index_JPN <- "D:/OneDrive - IATTC/IATTC/2026/SAC17/LL SSIs/Joint index/YFT/JPN_YFT/"
YFT_Index_KOR <- "D:/OneDrive - IATTC/IATTC/2026/SAC17/LL SSIs/Joint index/YFT/KOR_YFT/"

BET <- read.csv(paste0(BET_Index, "Index.csv")) %>% mutate(Index = "JPN+KOR")
BET_JPN <- read.csv(paste0(BET_Index_JPN, "Index.csv")) %>% mutate(Index = "JPN")
BET_KOR <- read.csv(paste0(BET_Index_KOR, "Index.csv")) %>% mutate(Index = "KOR")
YFT <- read.csv(paste0(YFT_Index, "Index.csv")) %>% mutate(Index = "JPN+KOR")
YFT_JPN <- read.csv(paste0(YFT_Index_JPN, "Index.csv")) %>% mutate(Index = "JPN")
YFT_KOR <- read.csv(paste0(YFT_Index_KOR, "Index.csv")) %>% mutate(Index = "KOR")

BET_Index <- rbind(BET, BET_JPN, BET_KOR) %>%
...
f7a <- ggplot(data = BET_Index) +
  ...
  theme_bw(18) +
  xlab("") + ylab("Relative CPUE\nCPUE relativa") +
  geom_hline(yintercept = 1) +
  coord_cartesian(xlim = c(2000, year), ylim = c(0, 2)) +
  ggtitle("BET") + theme(legend.position = c(.5, .9))

f7c <- ggplot(data = BET_Index) +
  # geom_ribbon(aes(x = Year, ymin = Low, ymax = High, fill = Index), alpha = 0.25) +
  geom_line(aes(x = Year, y = Std..Error.for.ln.Estimate., color = Index), linewidth = 1) +
  geom_point(aes(x = Year, y = Std..Error.for.ln.Estimate., color = Index), size = 2) +
  theme_bw(18) +
  xlab("") + ylab("Coefficient of variation\nCoeficiente de variación") +
  coord_cartesian(xlim = c(2000, year), ylim = c(0, 0.25)) +
  ggtitle("BET") + theme(legend.position = "none")


YFT_Index <- rbind(YFT, YFT_JPN, YFT_KOR) %>%
  mutate(Year = Time / 4 + 1974.875) %>%
  filter(Year > 2000) %>%
  group_by(Index) %>%
  mutate(
...
f7b <- ggplot(data = YFT_Index) +
  ...
  xlab("") + ylab("") +
  geom_hline(yintercept = 1) +
  coord_cartesian(xlim = c(2000, year), ylim = c(0, 8)) + 
  ggtitle("YFT") + theme(legend.position = c(.5, .9))

f7d <- ggplot(data = YFT_Index) +
  # geom_ribbon(aes(x = Year, ymin = Low, ymax = High, fill = Index), alpha = 0.25) +
  geom_line(aes(x = Year, y = Std..Error.for.ln.Estimate., color = Index), linewidth = 1) +
  geom_point(aes(x = Year, y = Std..Error.for.ln.Estimate., color = Index), size = 2) +
  theme_bw(18) +
  xlab("") + ylab("") +
  coord_cartesian(xlim = c(2000, year), ylim = c(0, 0.7)) +
  ggtitle("YFT") + theme(legend.position = "none")
```

```{r Figure 7bottom}
BET_Length <-
  "D:/OneDrive - IATTC/IATTC/2026/SAC17/Update_Assessment/Data/LL_LF/VAST_area_rw/"
YFT_Length <-
  "D:/OneDrive - IATTC/IATTC/2026/SAC17/LL SSIs/YFT/LF/VAST_area_rw/"
...
f7e <- ggplot(data = BET_Length) +
  geom_line(aes(x = Year, y = Mean_L)) +
  geom_point(aes(x = Year, y = Mean_L)) +
  theme_bw(18) +
  xlab("") + ylab("Relative average length\nTalla promedio relativa") +
  geom_hline(yintercept = BET_low, linetype = "dashed") +
  geom_hline(yintercept = BET_high, linetype = "dashed") +
  geom_hline(yintercept = 1) +
...
```{r Figure 7}
ggsave((f7a + f7b) / (f7c + f7d) / (f7e + f7f),
       file = "Figure7.pdf",
       height = 16,
       width = 16
)
