...
```{r Figure 5a}
Figure5a <-
  "D:/OneDrive - IATTC/IATTC/2024/SAC15/YFT/DEL SSIs/DEL_YFT75-Area-log-RW/"
...
```{r Figure 5b}
Figure5b <- "D:/OneDrive - IATTC/IATTC/2024/SAC15/YFT/DEL SSIs/"
...
```{r Figure 7top}
BET_Index <- "D:/OneDrive - IATTC/IATTC/2024/SAC15/Assessment/Data/LL_CPUE_operational/BET_vessel_hbf-gamma-rw-2knots/"
YFT_Index <- "D:/OneDrive - IATTC/IATTC/2024/SAC15/YFT/LL SSIs/YFT_vessel_hbf-lognormal-rw-200knots/"

BET <- read.csv(paste0(BET_Index, "Index.csv"))
YFT <- read.csv(paste0(YFT_Index, "Index.csv"))

BET <- BET %>% mutate(Species = "Bigeye")
YFT <- YFT %>% mutate(Species = "Yellowfin")

BET_Index <- BET %>%
  mutate(Year = Time / 4 + 1974.75) %>%
  filter(Year >= 2000) %>%
  mutate(
    CPUE = Estimate / mean(Estimate),
...
f7b <- ggplot(data = YFT_Index) +
  ...
  geom_hline(yintercept = YFT_high, linetype = "dashed") +
  geom_hline(yintercept = 1) +
  coord_cartesian(xlim = c(2000, year), ylim = c(0, 4.5)) + ggtitle("YFT")
```
